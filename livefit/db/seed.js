...
async function main() {
    ...
    // TODO：依照任務內容的規格寫入資料
    //   1. SKILL 三筆：重訓、瑜珈、飛輪
    const [weight, yoga, flying] = await skillRepo.save([
        { name: "重訓" },
        { name: "瑜珈" },
        { name: "飛輪" },
    ]);
    ...
    const [hi, mei] = await userRepo.save([
    ...
    await courseRepo.save([
        ...
        },
        {
            name: "晨間瑜珈",
            description: "瑜珈",
            start_at: "2026-09-03 08:00:00",
            end_at: "2026-09-03 09:00:00",
            max_participants: 10,
            user: mei,
            skill: yoga,
        },
